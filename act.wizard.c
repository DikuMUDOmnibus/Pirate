  ...
  struct descriptor_data *i;
	...
	if (!*buf)
		...
		if (!(victim = get_char_vis(ch,buf)))
			...
			act("$n disappears in a bright fire ball..", FALSE, victim, 0, 0, TO_ROOM);
			target = ch->in_room;
...
void do_start(struct char_data *ch)
{
	...
	send_to_char("Welcome. This is now your character in PirateMud,\n\rYou can now earn XP, and lots more...\n\r", ch);
